...
class Node:
    def __init__(self):
        ...
        self.next = None
        self.prev = None
def create_tree_visual(root):
    def create_node_visual(node, x, y, dx):
        nonlocal canvas
        if node is None:
            return

        canvas.create_oval(x - dx, y - dx, x + dx, y + dx, fill="white")
        canvas.create_text(x, y, text=str(node.data))

        if node.leftch:
            canvas.create_line(x, y + dx, x - dx * 2, y + 100)
            create_node_visual(node.leftch, x - dx * 2, y + 100, dx // 2)
        if node.rightch:
            canvas.create_line(x, y + dx, x + dx * 2, y + 100)
            create_node_visual(node.rightch, x + dx * 2, y + 100, dx // 2)

    window = tk.Tk()
    window.title("Threaded Binary Tree Visualization")
    canvas = tk.Canvas(window, width=800, height=600)
    canvas.pack()

    create_node_visual(root, 400, 100, 30)
